...
| --- | --- | --- |
| Collector contrib | `dinesh.gurumurthy/poc-dbm-only` | `/tmp/ddot-dbm-only` |
...
The DBM-only branch starts at fork `main` commit `16fa3257d56` and retains the
PostgreSQL receiver and DBM test assets from `f36c956fe80`. It has a dedicated
Collector build containing only components used by these tests. Recorded evidence
and earlier milestone hashes describe the original runs; separating the branch
does not constitute a new deployment or backend verification.

## Reproduce real receiver collection

Monitoring requires PostgreSQL 14+ with `pg_stat_statements` extension API 1.9+
for global reset detection. The [development stack](dev-stack/README.md) provisions
the extension, `dbm_monitor`, and `dbm_app`. Open its port-forward in a separate terminal:

```sh
kubectl --context kind-otel-dd -n dbm-otlp-dev port-forward service/postgres 25432:5432 --address 127.0.0.1
```
...
The runner reads the `dbm-otlp-dev/postgres:password` Secret reference
...
GOCACHE=/tmp/dbm-go-cache GOTMPDIR=/tmp GOPROXY=off GOSUMDB=off \
  python3 research/implementation/database-monitoring/otlp-intake/build-collector.py \
  --output /tmp/dbm-only-collector
python3 research/implementation/database-monitoring/otlp-intake/verify-collector-local.py \
  --binary /tmp/dbm-only-collector/collector/dbm-otlp-collector \
...
The [DBM-only manifest](builder-config.yaml) includes the PostgreSQL and OTLP
receivers, health check, memory limiter, resource and batch processors, OTLP HTTP
and debug exporters, and configuration providers. The builder checks for unwanted
Datadog components, HTTP forwarding, transform processing, and `pkg/trace`
dependencies. Container packaging is handled by the development stack's deploy script.
