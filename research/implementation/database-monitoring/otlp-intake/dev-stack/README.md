...
Run from the Collector PoC branch checkout. Prerequisites: Docker, kind, kubectl,
...
The deployed binary is the previously built and tested generic Collector:
...
To rebuild it, use the [Collector build instructions](../README.md#build-and-exercise-the-collector-service)
with `--generic-only --skip-image`. Then deploy, passing the resulting binary:
...
python3 research/implementation/database-monitoring/otlp-intake/dev-stack/deploy.py \
  ...
  --collector-binary /tmp/dbm-local-collector-epochs/collector/ddot-products-collector
